                  ...
                  const cnnumber proj[16])
{
    ...
    glUseProgram(mat->gpu_handler.gl_shader);

    glUniformMatrix4fv(glGetUniformLocation(mat->gpu_handler.gl_shader, "u_model"),
                       1, GL_FALSE, model);
    glUniformMatrix4fv(glGetUniformLocation(mat->gpu_handler.gl_shader, "u_view"),
                       1, GL_FALSE, view);
    glUniformMatrix4fv(glGetUniformLocation(mat->gpu_handler.gl_shader, "u_proj"),
                       1, GL_FALSE, proj);

    /* Phong material */
    glUniform4f(glGetUniformLocation(mat->gpu_handler.gl_shader, "u_color"),
                ((mat->color & 0xff000000) >> 24) / 255.0f,
            ((mat->color & 0x00ff0000) >> 16) / 255.0f,
            ((mat->color & 0x0000ff00) >> 8) / 255.0f,
            (mat->color & 0x000000ff) / 255.0f);
    glUniform1f(glGetUniformLocation(mat->gpu_handler.gl_shader, "u_ambient"),   mat->gpu_handler.gl_shader);
    glUniform1f(glGetUniformLocation(mat->gpu_handler.gl_shader, "u_diffuse"),   mat->diffuse);
    glUniform1f(glGetUniformLocation(mat->gpu_handler.gl_shader, "u_specular"),  mat->specular);
