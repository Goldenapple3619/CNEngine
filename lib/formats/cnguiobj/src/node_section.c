...
static uint64_t build_node_section_size(struct engine_object_file_section_writer_ctx_s *self)
{
    return ((
        sizeof(uint32_t) + // parent
        sizeof(uint32_t) + // id
        sizeof(uint32_t) + // type
        sizeof(uint32_t)) * // text
            ((struct generic_vector_s *)self->_v)->size // count
    );
}

static char *build_node_section(struct engine_object_file_section_writer_ctx_s *self, const struct engine_object_file_writer_ctx_s *wctx, struct generic_map_s *strndx)
{
    struct generic_vector_s *vec = self->_v;
    struct gui_element_s *temp;
    char *generated = malloc(sizeof(char) * build_node_section_size(self));
...
static char *reconstruct_node_section_strndx(char *rw_content, uint64_t content_size, const CNAssetReader *reader, struct generic_map_s *new_strndx)
{
    uint64_t pos = 0;
    uint32_t val = 0;
    ...
    while (pos < content_size) {
        val = add_str_table(object_file_reader_get_string(reader, reader->read_handler.u32((const void *)(rw_content + pos))), new_strndx); // parent
        if (val == 0xFFFFFFFFu)
            return (NULL);
        ...
        val = add_str_table(object_file_reader_get_string(reader, reader->read_handler.u32(rw_content + pos)), new_strndx); // text
        if (val == 0xFFFFFFFFu)
            return (NULL);
        (void)writter_u32(rw_content + pos, val);
        pos += sizeof(uint32_t);
...
static char *convert_node_section_endian(char *rw_content, uint64_t content_size, const CNAssetReader *reader, engine_wrt_endian endian)
{
    uint64_t pos = 0;
...
void init_node_section_registry(struct section_registry *reg)
{
    ...
    reg->section_type = ENGINE_SEC_GUI_NODES;

    reg->strndx_reconstructor = &reconstruct_node_section_strndx;
    reg->data_builder = &build_node_section;
    reg->size_compute = &build_node_section_size;
    reg->endian_converter = &convert_node_section_endian;
}
